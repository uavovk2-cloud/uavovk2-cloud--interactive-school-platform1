...
async function dbRequired(env) {
  ...
  // D1 is more reliable here when each DDL statement is prepared separately
  // and executed as one batch, instead of sending a multi-statement SQL string
  // through DB.exec().
  if (!schemaReady) {
    await env.DB.batch([
...
async function state(db, lesson, teacher = false) {
  ...
  const result = {
    lesson: {
      ...
      currentQuestionId: lesson.current_question_id
    },
...
async function api(request, env) {
  ...
  if (path === "/api/health" && method === "GET") {
    return json({ ok: true, database: true, version: "mvp-2" });
  }
  ...
  if (path === "/api/lessons" && method === "POST") {
    ...
    return json({
      ...
      teacherToken: lesson.teacher_token
    });
  }
  ...
  if (m && method === "POST") {
    ...
    await db.prepare(
      "INSERT INTO students (id, lesson_id, name) VALUES (?1, ?2, ?3)"
  ...
  if (m && method === "GET") {
    ...
    const student = await db.prepare(
      "SELECT id FROM students WHERE id = ?1 AND lesson_id = ?2"
    ).bind(m[2], lesson.id).first();
  ...
  if (m && method === "POST") {
    const lesson = await findLesson(db, m[1]);
    if (!lesson || !lesson.current_question_id) {
      return json({ error: "Зараз немає активного питання." }, 400);
    }
    ...
    const student = await db.prepare(
      "SELECT id FROM students WHERE id = ?1 AND lesson_id = ?2"
    ).bind(studentId, lesson.id).first();

    if (!student) return json({ error: "Учня не знайдено." }, 403);
    if (!answerText) return json({ error: "Обери або введи відповідь." }, 400);

    await db.prepare(`
      INSERT INTO answers (id, question_id, student_id, answer_text)
      VALUES (?1, ?2, ?3, ?4)
      ON CONFLICT(question_id, student_id)
      DO UPDATE SET answer_text = excluded.answer_text, created_at = CURRENT_TIMESTAMP
  ...
  if (m && method === "POST") {
    ...
    const options = Array.isArray(data.options)
      ? data.options.map(String).map(v => v.trim()).filter(Boolean)
      : [];

    const count = await db.prepare(
      "SELECT COUNT(*) AS n FROM questions WHERE lesson_id = ?1"
    ...
    await db.prepare(`
      INSERT INTO questions (id, lesson_id, type, text, options_json, sort_order)
      VALUES (?1, ?2, ?3, ?4, ?5, ?6)
    `).bind(
      question.id,
      lesson.id,
  ...
  if (m && method === "POST") {
    ...
    if (action === "start") {
      ...
      const questions = await db.prepare(
        "SELECT id FROM questions WHERE lesson_id = ?1 ORDER BY sort_order, created_at"
      ).bind(lesson.id).all();
      ...
      await db.prepare(
        "UPDATE lessons SET status = 'live', current_question_id = ?1 WHERE id = ?2"
      ).bind(next.id, lesson.id).run();
    } else if (action === "stop") {
